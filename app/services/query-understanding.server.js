...
- TRUE — ALWAYS — when the user pastes a multi-field spec sheet (e.g. "Brand: X Series: Y Length: 20m Connector 1: M12 ..." or "Schmersal Lever Thermoplastic Cable entry M 20 x 1.5 Limit Switch 60.5 mm*40.5 mm"). They are trying to find a product matching those specs — extract brand/category/spec_values from the paste and search. Do NOT treat a spec paste as a Q&A turn even if no carry-over context exists; the user is shopping by specification, not asking ABOUT a prior product.
...
- Tie-breakers: if the user references a previously-shown product implicitly ("it", "that one", "this", "the one above", "the third one") and no new SKU is mentioned, prefer FALSE. If a SKU or product name not in recent_conversation appears, prefer TRUE. If the turn is a spec paste (3+ field:value-style segments OR a dense list of nouns + dimensions), prefer TRUE — that's a new product search.
